...
contract StakeHolderFundTimeLock is Ownable {
    ...
    uint internal constant WITHDRAWAL_INTERVAL_SECS = 3600 * 24 * 30.5;
    // Total amount of withdrawal for one "interval"
    uint internal constant TOTAL_WITHDRAWAL_AMT_PER_INTERVAL = 380555e18;
    ...
    constructor (address _nature, address[] memory wallets, address _founder, address _cofounder, uint delaySecs) public
    {
        require(_nature != address(0), "invalid nature address");
        nature = IBEP20(_nature);
        revertZeroAddress(_cofounder);
        revertZeroAddress(_founder);
        founder = _founder;
        cofounder = _cofounder;
        for (uint i = 0 ; i < wallets.length; i++) {
            revertZeroAddress(wallets[i]);
            Teams.push(Team(wallets[i], true));
            teamNum = teamNum + 1;
            teamIds[wallets[i]] = teamNum;
        }
    ...
    function withdraw() public {
        ...
        require(timePast >= intervalSecs, "nature locked");

        uint balance = nature.balanceOf(address(this));
        ...
        uint share = amount.mul(5).div(teamNum).div(15);
        uint founderShare = amount.mul(5).div(15);
        for (uint i = 0 ; i < Teams.length; i ++) {
            // for an active co-founder
            if(Teams[i].isActive == true) {
                amount = amount.sub(share);
                nature.safeTransfer(Teams[i].wallet, share);
            }
        }
        // Founder gets 450/1170
        nature.safeTransfer(founder, founderShare);
        amount = amount.sub(founderShare);
        nature.safeTransfer(cofounder,amount);

    }

    function addTeam(address wallet) public onlyOwner {
        revertZeroAddress(wallet);

        uint id = teamIds[wallet];
        if (id == 0) {
            Teams.push(Team(wallet, true));
            teamNum = teamNum + 1;
            teamIds[wallet] = teamNum;
        } else {
            Teams[id - 1].isActive = true;
        }
    }

    function removeTeam(address wallet) public onlyOwner {
        revertZeroAddress(wallet);
    ...
    function updateCoFounder(address _coFounder) public onlyOwner {
        cofounder = _coFounder;
    }
